...
public:
	...
	bool operator== (const DynamicArray<T>& a) const;
...
template<typename T>
bool DynamicArray<T>::operator==(const DynamicArray<T> &a) const
{
	bool ret = (a.getLength() == m_size);
	for(int i = 0; i < a.getLength() && ret; ++i)
		ret = (a[i] == m_data[i]);
	return ret;
}
